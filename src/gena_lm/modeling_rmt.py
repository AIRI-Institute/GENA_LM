...
class RMTEncoderForSequenceClassification(torch.nn.Module):
    ...
    def extend_word_embeddings(self, num_mem_tokens):
        ...
        vocab_size = self.model.base_model.get_input_embeddings().weight.shape[0]
...
class RMTEncoderExpression(RMTEncoderForSequenceClassification):
    ...
    def forward(self,
        ...
        if self.mixed_length_ratio > 0.0 and self.model.training and random.random() > self.mixed_length_ratio:
            max_n_segments = random.randint(2, max_n_segments) if max_n_segments > 1 else max_n_segments


        for seg_num, (segment_input_ids, segment_labels, segment_labels_mask) in enumerate(
            zip(segmented, segmented_labels, segmented_labels_mask)
        ):
    ...
    def pad_and_segment(self, input_ids, labels=None, labels_mask=None, tpm = None):
        ...
        for batch_index, (seq, seq_labels, seq_labels_mask) in enumerate(zip(input_ids, batch_labels, batch_labels_mask)):
            content_tokens_mask = (
            ...
            if seq_labels is not None:
                ...
                seq_labels = seq_labels[:self.segment_size * self.rmt_config['max_n_segments']]
            if seq_labels_mask is not None:
                seq_labels_mask = seq_labels_mask[content_tokens_mask]
                seq_labels_mask = seq_labels_mask[:self.segment_size * self.rmt_config['max_n_segments']]
        
            n_seg = math.ceil(len(seq) / self.segment_size)
            input_segments = torch.chunk(seq, n_seg)
            labels_segments = torch.chunk(seq_labels, n_seg) if seq_labels is not None else None
            labels_mask_segments = torch.chunk(seq_labels_mask, n_seg) if seq_labels_mask is not None else None
            ...
            for i, seg in enumerate(input_segments):
                segs_padded.append(
                    self.pad_add_special_tokens(
                        seg, self.rmt_config['input_size'],
                        add_to='inputs',
                        tpm=current_tpm
                    )
                )
                if labels_segments is not None:
                    ...
                    segs_labels_padded.append(
                        self.pad_add_special_tokens(
                            seg_labels, self.rmt_config['input_size'],
                            add_to='labels',
                            tpm=current_tpm
                        )
                    )
                if labels_mask_segments is not None:
                    ...
                    segs_mask_padded.append(
                        self.pad_add_special_tokens(
                            seg_labels_mask, self.rmt_config['input_size'],
                            add_to='labels_mask',
                            tpm=current_tpm
                        )
                    )
        ...
        if any(len(x) > 0 for x in segmented_batch_labels_mask):
            segmented_batch_labels_mask = [
                [s[::-1][i] if len(s) > i else None for s in segmented_batch_labels_mask]
                for i in range(self.rmt_config['max_n_segments'])
            ][::-1]
    ...
    def pad_add_special_tokens(self, tensor, segment_size, add_to='inputs', tpm=None):
        ...
        if add_to == 'inputs':
            ...
            if pad_size > 0:
                final_tensor = F.pad(final_tensor, (0, pad_size), value=self.pad_token_id)
            return final_tensor
...
class RMTEncoderExpression_scibert(RMTEncoderForSequenceClassification):
    def __init__(self, base_model, **rmt_kwargs):
        super().__init__(base_model, **rmt_kwargs)
        self.rmt_config['sum_loss'] = True
        self.mixed_length_ratio = self.rmt_config.get('mixed_length_ratio', 0.0)
    ...
    def forward(self,
        ...
        for seg_num, (segment_input_ids, segment_labels, segment_labels_mask) in enumerate(
            ...
            if desc_input_ids is not None:
                seg_kwargs['desc_input_ids'] = desc_input_ids[non_empty_mask]

            if desc_attention_mask is not None:
                seg_kwargs['desc_attention_mask'] = desc_attention_mask[non_empty_mask]
...
class RMTEncoderExpression_cells(RMTEncoderForSequenceClassification):
    def __init__(self, base_model, **rmt_kwargs):
        super().__init__(base_model, **rmt_kwargs)
        self.rmt_config['sum_loss'] = True
        self.mixed_length_ratio = self.rmt_config.get('mixed_length_ratio', 0.0)
    ...
    def forward(self,
        ...
        kwargs = {
            ...
            'dataset_flag': dataset_flag,
                ...
                # --- reshape (B, N, ...) -> (B*N, ...) BEFORE computing bs ---
        # input_ids: (B, N, L) -> (B*N, L)
        if input_ids is not None and input_ids.dim() == 3:
            B, N, L = input_ids.shape
            input_ids = input_ids.reshape(B * N, L)
            kwargs['input_ids'] = input_ids  # keep kwargs in sync

        # token_type_ids: (B, N, L) -> (B*N, L)
        ...
        if attention_mask is not None and attention_mask.dim() == 3:
            attention_mask = attention_mask.reshape(B * N, L)
            kwargs['attention_mask'] = attention_mask

        # labels: (B, N, L, 1) -> (B*N, L, 1)
        if labels is not None:
            if labels.dim() == 4:
                B, N, L, C = labels.shape
                labels = labels.reshape(B * N, L, C)
                kwargs['labels'] = labels

        # labels_mask: (B, N, L, 1) -> (B*N, L, 1)
        if labels_mask is not None:
            if labels_mask.dim() == 4:
                B, N, L, C = labels_mask.shape
                labels_mask = labels_mask.reshape(B * N, L, C)
                kwargs['labels_mask'] = labels_mask

                # desc_vectors: (B, N, D) -> (B*N, D)
        if desc_vectors is not None:
            if desc_vectors.dim() == 3:
                B, N, D = desc_vectors.shape
                desc_vectors = desc_vectors.reshape(B * N, D)
            else:
                raise ValueError(f"Unexpected desc_vectors shape: {tuple(desc_vectors.shape)}")
            kwargs['desc_vectors'] = desc_vectors
        ...
        if tpm is not None:
            if tpm.dim() == 3:
                B, N, C = tpm.shape
                if C != 1:
                    raise ValueError(f"tpm is expected to have last dim==1, got {C}")
                tpm = tpm.reshape(B * N, C)
...
class RMTEncoderExpression_description(RMTEncoderForSequenceClassification):
    def __init__(self, base_model, **rmt_kwargs):
        super().__init__(base_model, **rmt_kwargs)
        self.rmt_config['sum_loss'] = True
        self.mixed_length_ratio = self.rmt_config.get('mixed_length_ratio', 0.0)
...
class RMTEncoderExpressionModernGENA(RMTEncoderForSequenceClassification):
    def __init__(self, base_model, **rmt_kwargs):
        super().__init__(base_model, **rmt_kwargs)
        self.rmt_config['sum_loss'] = True
        self.mixed_length_ratio = self.rmt_config.get('mixed_length_ratio', 0.0)
    ...
    def forward(self,
        ...
        for seg_num, (segment_input_ids, segment_labels, segment_labels_mask) in enumerate(
            ...
            # inputs_embeds_ = self.model.embeddings(input_ids_)
            inputs_embeds_ = self.model.embeddings.token_embeddings(input_ids_)
            inputs_embeds_[:, self.memory_position] = memory[non_empty_mask]
