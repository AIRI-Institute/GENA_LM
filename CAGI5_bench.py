...
class CAGI5_bench():
    ...
    def initialize_dataset(self, 
        ...
        self.dataset = KircherDataset(elements=elements, length=length, mode=mode, cell_type=cell_type, transform=_make_transform() if transform is None else transform, target_transform=target_transform, root=root)
    ...
    def run_inference(self):
        ...
        with torch.autocast(device_type='cuda'), torch.inference_mode():
            ...
            self.corr = {el : torch.corrcoef(value.float().T) for el, value in self.true_vs_pred.items()}
