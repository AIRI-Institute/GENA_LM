...
parser.add_argument('--aug_shift_min_values', type=str, default='[0,0]')
parser.add_argument('--aug_shift_max_values', type=str, default='[0,0]')
parser.add_argument('--aug_rc_values', type=str, default="['keep','rc']", help='keep | rc | rnd')
...
if __name__ == '__main__':
    ...
    if not args.augment:
        raise RuntimeError('This script supports only evaluation with augmentations: --augment')
    ...
    # get augmentations params
    aug_shift_min = ast.literal_eval(args.aug_shift_min_values)
    aug_shift_max = ast.literal_eval(args.aug_shift_max_values)
    aug_rc = ast.literal_eval(args.aug_rc_values)
    n_augs = len(aug_shift_min)
    assert n_augs == len(aug_shift_max) == len(aug_rc)

    datasets = {'train': [], 'valid': [], 'test': []}

    for data_path, sn in zip([args.data_path, args.valid_data_path, args.test_data_path],
                             ['train', 'valid', 'test']):
        if data_path:
            logger.info(f'preparing {sn} data from: {data_path}')
            data_path = Path(data_path).expanduser().absolute()
            # dataset per augmentation
            for i in range(n_augs):
                dataset = EnformerDataset(tokenizer, data_path, max_seq_len=args.input_seq_len,
                                          ...
                                          augment=args.augment, aug_rc=aug_rc[i],
                                          aug_shift_min=aug_shift_min[i], aug_shift_max=aug_shift_max[i],
                                          )
                logger.info(f'[{i}] {sn} shift_min: {aug_shift_min[i]} shift_max: {aug_shift_max[i]} '
                            f'rc: {aug_rc[i]} dataset len: {len(dataset)}')
                datasets[sn] += [DataLoader(dataset, batch_size=args.batch_size,  **kwargs)]
    ...
    for sn in datasets:
        augs_dataloaders = datasets[sn]
        if len(augs_dataloaders) > 0:
            ...
            if args.augment:
                sn_detailed = f'{sn_detailed}_n_eval_augs_{n_augs}'

            if args.save_predictions is not None:
                preds_filename = f'{args.save_predictions}_{sn_detailed}.h5py'
            ...
            with torch.no_grad():
                data_iterators = [iter(dl) for dl in augs_dataloaders]
                for i in tqdm(range(len(augs_dataloaders[0])), total=len(augs_dataloaders[0]), desc=sn):
                    preds = []
                    labels = []
                    for j, dl in enumerate(data_iterators):
                        batch = next(dl)
                        ...
                        if not is_rmt:
                            ...
                            label = batch['labels'][batch['labels_mask']]
                        else:
                    ...
                    for x in labels:
                        if not (x == labels[0]).all():
                            raise RuntimeError('labels check across augmentations failed')
                    preds = torch.stack(preds).mean(dim=0).cpu()
                    labels = labels[0].cpu()

                    corr_coef(preds=preds, target=labels)
                    if args.save_predictions is not None:
                        n_new = pred.shape[0]
