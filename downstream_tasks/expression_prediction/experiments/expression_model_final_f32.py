...
class ExpressionCounts(nn.Module):
    ...
    def __init__(
        ...
        self.loss_fct.to(dtype=torch.bfloat16)
        self.weight = weight
    ...
    def forward(
        ...
        sequence_output[:, 0, :] = sequence_output[:, 0, :].clone() + desc_output
        ...
        if labels is not None:
            if labels.dtype != logits.dtype:
                labels_reshaped = labels.to(dtype=logits.dtype)
            if labels_mask is not None and labels_mask.dtype != logits.dtype:
                labels_mask_reshaped = labels_mask.to(dtype=logits.dtype)
