...
class ExpressionCounts(nn.Module):
    ...
    def __init__(
        ...
        # 1) DNA model (GENA) 
        if hf:
            if "moderngena" in hf_model_name.lower():
                from modernbert_utils import load_flexbert_model
                # self.logger.info(f"Loading SUPER ModernBERT model from {hf_model_name}")
        ...
        # self.decoder = load_flexbert_model(hf_model_name_decoder, logger=self.logger)
    ...
    def forward(
        ...
        seq_compact = bert_outputs                  # (U_inp, L, H)
