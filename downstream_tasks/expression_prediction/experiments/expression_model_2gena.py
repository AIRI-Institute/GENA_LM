...
class ExpressionCounts(BertPreTrainedModel):
    ...
    def __init__(
        ...
        second_bert_cpt = '/mnt/nfs_dna/DNALM/trained_models/bert_base_512_t2t_1000G_bs256_lr_1e-04_fp16/model_best.pth',
        cell_type_specific_loss_fn = None,
        text_model = None,
        hf: bool = False,
        hf_model_name: str = "AIRI-Institute/gena-lm-bert-large-t2t",
        second_hf: bool = True,
        second_hf_model_name: str = "AIRI-Institute/gena-lm-bert-large-t2t",
    ):
        ...
        # 3) Вторая GENA вместо Encoder
        if second_hf:
            second_hf_config = BertConfig.from_pretrained(second_hf_model_name)
            self.second_bert = BertModel(second_hf_config, add_pooling_layer=False)
            second_weights_path = cached_file(second_hf_model_name, "pytorch_model.bin")
            second_state_dict = torch.load(second_weights_path, map_location="cpu")
            second_updated_state_dict = {
                k.replace("bert.", ""): v for k, v in second_state_dict.items() if k.startswith("bert.")
            }
            second_missing_k, second_unexpected_k = self.second_bert.load_state_dict(second_updated_state_dict, strict=False)
            if len(second_missing_k) != 0:
                print(f"Second GENA: {second_missing_k} were not loaded from checkpoint! These parameters were randomly initialized.")
            if len(second_unexpected_k) != 0:
                print(f"Second GENA: {second_unexpected_k} were found in checkpoint, but model is not expecting them!")
        else:
            self.second_bert = BertModel(config, add_pooling_layer=False)
            second_checkpoint = torch.load(second_bert_cpt, map_location="cpu")
            second_state_dict = second_checkpoint["model_state_dict"]
            second_updated_state_dict = {k.replace("bert.", ""): v for k, v in second_state_dict.items()}
            second_missing_k, second_unexpected_k = self.second_bert.load_state_dict(second_updated_state_dict, strict=False)
            if len(second_missing_k) != 0:
                print(f"Second GENA: {second_missing_k} were not loaded from checkpoint! These parameters were randomly initialized.")
            if len(second_unexpected_k) != 0:
        ...
        for p in self.second_bert.embeddings.word_embeddings.parameters():
    ...
    def forward(
        ...
        # Прогоняем через вторую GENA
        second_bert_outputs = self.second_bert(
            inputs_embeds=seq_out_flat,
            attention_mask=attention_mask.repeat(N, 1) if attention_mask is not None else None,
            return_dict=True,
        )
        encoder_output = second_bert_outputs.last_hidden_state  # (B*N, seq_len, hidden_size)
