# Got this file from Alena (TG), original name: expression_dataset_tss_shifted.py
...
from datasets.src.utils import convert_fm_relative_path_to_absolute_path
import pickle
...
class ExpressionDataset(Dataset):
    ...
    def tokenize_genome(self, i):
        ...
        if reverse == 0: # forward strand
            start_gene = start - sum(t[2] for t in token_lengths) #TODO: check if we need substraction here
            assert sum(t[2] for t in token_lengths)==0, "Something is wrong here"
        else: # reverse strand
            start_gene = end
        ...
        if reverse == 0:
            try:
                chrom_length = self.sequences.get_reference_length(chrom)
                fetch_start = min(start + self.shift,chrom_length - 50)
                fetch_end = min(start + self.shift + 9 * 1500,chrom_length)
                assert fetch_start < fetch_end, "Something is wrong here"
            ...
            try:
                fetch_start = max(0,start - 9 * 1500 - self.shift)
                fetch_end = max(50,start - self.shift)
                assert fetch_start < fetch_end, "Something is wrong here"
                sequence = self.sequences.fetch(chrom, fetch_start, fetch_end).upper()
                sequence = self.reverse_complement(sequence)
            except ValueError as e:
        ...
        token_lengths_df['start'] = token_lengths_df['length'].cumsum().shift(fill_value=0) + fetch_start  
