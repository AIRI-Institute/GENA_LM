...
class ExpressionDataset(Dataset):
    def __init__(
        ...
        norm_bw = False,
        text_tokenizer: str = "allenai/scibert_scivocab_uncased",
        ...
        # Кэш для токенизированных описаний
        ...
        if os.path.exists(self.desc_h5_cache_path):
            ...
            self._load_descriptions_from_json(targets_path)
    ...
    def _load_descriptions_from_json(self, targets_path):
        ...
        for idx, row in df.iterrows():
            ...
            with open(full_metadata_path, "r", encoding="utf-8") as f:
                meta = json.load(f)
            desc = self._make_description_from_json(meta, description_id, full_metadata_path)
            self.text_data[description_id] = desc
        self.text_data_keys = set(self.text_data.keys())

    def _make_description_from_json(self, meta, description_id, meta_path):
        import re
    ...
    def _precompute_descriptions(self):
