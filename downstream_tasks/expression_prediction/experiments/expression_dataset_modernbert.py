...
#        assert not "strand" in reverse_genes.columns.values, "reverse_intervals_path must not contain strand column"
    ...
    def __getitem__(self, idx):
        ...
        l = min(len(input_ids), self.gen_max_seq_len)
        cls_id = getattr(self.gen_tokenizer, "cls_token_id", None)
        sep_id = getattr(self.gen_tokenizer, "sep_token_id", None)
        if cls_id is None or sep_id is None:
            raise ValueError("У токенайзера не найдены CLS/SEP (и BOS/EOS). Укажите корректный токенайзер.")
        input_ids[0] = cls_id
        input_ids[l-1] = sep_id
        ...
        if self.tpm:
            ...
            features["labels"][0, :] = torch.from_numpy(tpm_values).to(features["labels"].dtype)
            features["labels_mask"][0, :] = torch.from_numpy(~np.isnan(tpm_values))
