...
class ExpressionDatasetMPRA(Dataset):
    def __init__(
        self,
        gen_tokenizer,
        targets_path: str,
        ...
        self.sequence_ids = self.sequences.references
        self.hash_prefix = os.path.dirname(self.genome) 
        ...
        if self.tpm:
            ...
            if os.path.exists(tpm_hash_path):
                ...
                self.tpm_lookup = {}
                for key, (_, tpm_path) in self.paths.items():
                    self.logger.info(f"Reading tpm from {tpm_path}")
                    # Читаем файл с правильными типами данных
                    tpm = pd.read_csv(tpm_path, sep='\t', index_col="name")
                    # Преобразуем только числовые столбцы в float32
                    numeric_cols = tpm.select_dtypes(include=[np.number]).columns
                    tpm[numeric_cols] = tpm[numeric_cols].astype(np.float32)
                    self.tpm_lookup[key] = tpm
    ...
    def _compute_valid_indices(self):
        ...
        for idx in range(len(self.sequence_ids)):
            sequence_id = self.sequence_ids[idx]
            
            # Проверяем наличие TPM значений для всех ключей
            has_all_tpm_data = True
            for key in self.paths.keys():
                if sequence_id not in self.tpm_lookup[key].index:
                    has_all_tpm_data = False
                    break
                
            # Добавляем индекс только если есть TPM данные для всех ключей
            if has_all_tpm_data:
                self.valid_indices.append(idx)
                
        self.logger.info(f"Found {len(self.valid_indices)} valid samples out of {len(self.sequence_ids)}")

    def get_hash_path(self):
        m = hashlib.blake2b(digest_size=8)
        m.update(str('tokens_mpra').encode("utf-8"))
    ...
    def get_tpm_hash_path(self):
        ...
        hash_suffix = m.hexdigest()
        return str(self.hash_prefix) + ".tpm." + hash_suffix
    ...
    def precompute_tokenization(self):
        ...
        try:
            with h5py.File(temp_path, "w") as h5f:
                pbar = tqdm.tqdm(total=len(self.sequence_ids), desc="Tokenizing sequences")
                for idx in range(len(self.sequence_ids)):
                    sequence_id = self.sequence_ids[idx]
                    tokens_df = self.tokenize_sequence(idx)
                    
                    gene_group = h5f.create_group(sequence_id)
    ...
    # tokenize sequence
    def tokenize_sequence(self, i):
        sequence_id = self.sequence_ids[i]
        sequence = self.sequences.fetch(sequence_id).upper()[15:-15]
        ...
        tokens = encoded_sequence['input_ids']
    ...
    def __getitem__(self, idx):
        ...
        sequence_id = self.sequence_ids[original_idx]
        gene_group = self.h5_cache[sequence_id]
        ...
        features = {
            ...
            "gene_id": [sequence_id] * self.n_keys,
            "name": sequence_id,
        }
        ...
        if self.tpm:
            for i, key in enumerate(selected_keys):
                if sequence_id in self.tpm_lookup[key].index:
                    tpm_values[i] = self.tpm_lookup[key].loc[sequence_id].iloc[0]
    ...
    def describe(self):
        return f"ExpressionDataset(n_genes={len(self.valid_indices)}, n_cell_types={len(self.paths.keys())}, n_chunks={self.n_cell_chunks}, tpm={self.tpm})"
