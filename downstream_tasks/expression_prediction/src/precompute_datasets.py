...
# ---------- helpers: locate repo root (directory that contains 'downstream_tasks') ----------
...
# detect repo root relative to this script OR CWD
...
if _repo is not None:
    ...
    # also set/extend PYTHONPATH so child processes inherit it
...
# show a short sys.path head to debug
for i, p in enumerate(sys.path[:5], 1):
    log.debug(f"sys.path[{i}]: {p}")

# ---------- HOME_PATH resolver (for ${HOME_PATH} in YAML) ----------
_home_default = os.environ.get("HOME_PATH", os.path.expanduser("~"))
OmegaConf.register_new_resolver("HOME_PATH", lambda: _home_default)

# --- утилита: аккуратно подмешать shared_dataset_params в датасетный конфиг ---
...
def _init_one_dataset(cfg_dict):
    """
    Возвращает dict-статус: {'name': str, 'ok': bool, 'secs': float, 'error': Optional[str]}
    """
    t0 = time.time()
    name = _short_ds_name_from_cfg_dict(cfg_dict)

    # limit threads (avoid 1 proc x 32 threads)
    ...
    # ensure PYTHONPATH in workers too
    if _repo is not None:
        repo_str = str(_repo)
        if repo_str not in sys.path:
            sys.path.insert(0, repo_str)
    ...
    try:
        wlog.info("start init")
        logging.getLogger().setLevel(logging.ERROR)

        cfg = OmegaConf.create(cfg_dict)
        ds = hydra_instantiate(cfg)  # -> создаст/прочитает HDF5-кэш
        _ = len(ds)                  # "ping" ленивых мест
        del ds

        secs = time.time() - t0
        wlog.info(f"done in {secs:.2f}s")
...
def main():
    ...
    parser.add_argument("--include_valid", action="store_true", help="Также прогреть valid_* датасеты")
    parser.add_argument("--n_keys", type=int, default=13, help="Жёстко выставить n_keys для всех датасетов (default: 13)")
    parser.add_argument("--log_file", type=str, default=None, help="Писать логи ещё и в файл")
    parser.add_argument("-v", "--verbose", action="store_true", help="Более подробные логи (DEBUG)")
    ...
    # merge shared into each dataset config
    ...
    # finalize: set n_keys = args.n_keys for ALL datasets (train + optional valid)
    ...
    if not merged_all:
        log.error("Нет датасетов для инициализации — выходим.")
        return
    ...
    log.info(f"Стартуем {workers} процессов для {total} датасетов")
    ...
    with ctx.Pool(processes=workers) as pool:
        for status in pool.imap_unordered(_init_one_dataset, tasks):
            if status["ok"]:
                ok_cnt += 1
    ...
    log.info(f"Готово: success={ok_cnt}, fail={fail_cnt}, всего={total}, время={total_secs:.2f}s")
