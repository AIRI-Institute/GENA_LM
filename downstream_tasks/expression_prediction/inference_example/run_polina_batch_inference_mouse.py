...
DEFAULT_GENA_HOME = Path(os.environ.get("GENA_HOME", REPO_ROOT))
DEFAULT_TASK_ROOT = Path(os.environ.get("TASK_ROOT", DEFAULT_GENA_HOME))
DEFAULT_DATA_ROOT = Path(os.environ.get("DATA_ROOT", DEFAULT_GENA_HOME / "data"))
...
def main():
    ...
    model_class_path = str(
        experiment_config["args_params"]["model_cls"]
    )

    module_name, class_name = model_class_path.split(":", 1)
    model_module = importlib.import_module(module_name)
    ExpressionCounts = getattr(model_module, class_name)
