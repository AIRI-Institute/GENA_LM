...
def score_predictions(ground_truth, predictions, path_to_selected,
...
def main():
    parser = argparse.ArgumentParser(description='Score predictions against ground truth data for cell type specificity.')
    parser.add_argument('--ground_truth', type=str, required=True, help='Path to CSV with ground truth data')
    parser.add_argument('--predictions', type=str, required=True, help='Path to CSV with predicted values')
    parser.add_argument('--selected', type=str, required=True, help='Path to CSV file containing selected genes and targets')
    parser.add_argument('--output', type=str, help='Path to save results (optional)')
    parser.add_argument('--need_log', type=bool, default=True, help='Log-transform expression values')
    parser.add_argument('--pseudo_count', type=float, default=0.01, help='Pseudo count for log-transform')
    ...
    try:
        results = score_predictions(
            ...
            args.predictions,
            args.selected,
            need_log=args.need_log,
            pseudo_count=args.pseudo_count,
            logger=None  # No logger for command line usage
        )

        # Print results
        print("\nScoring Results:")
        print(f"Number of genes evaluated: {results['genes_number']}")
        print(f"Number of cell types/conditions: {results['ct_number']}")
        print(f"Number of expression pairs: {results['expression_pairs']}")
        print(f"Deviation correlation coefficient: {results['deviation_r']:.4f}")
