...
class BertTailEncoder(nn.Module):
    def __init__(self, bert_layers):
        super().__init__()
        # делаем глубокую копию выбранных слоёв, чтобы не шарить веса с self.bert
        self.layers = nn.ModuleList([copy.deepcopy(l) for l in bert_layers])

    def forward(self, x):
        h = x
        for layer in self.layers:
            out = layer(hidden_states=h)   # attention_mask НЕ передаём
...
class ExpressionCounts(BertPreTrainedModel):
    ...
    def __init__(
        ...
        take_k = min(num_encoder_layers, len(self.bert.encoder.layer))  # обычно 3
        tail_layers = self.bert.encoder.layer[-take_k:]                  # последние k слоёв GENA
        self.transformer_encoder = BertTailEncoder(tail_layers)
        ...
        self.post_encoder_ln = nn.LayerNorm(bert_cfg.hidden_size, eps=getattr(config, "layer_norm_eps", 1e-12))
