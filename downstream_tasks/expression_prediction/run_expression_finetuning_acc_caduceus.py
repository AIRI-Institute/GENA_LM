...
from downstream_tasks.expression_prediction.expression_dataset_caduceus import worker_init_fn
...
def main():
   ...
   # alogger = get_logger('')  # accelerate logger (привязан к процессу)
    ...
    def collate_fn(batch):
        pad_keys = ['input_ids','labels', 'labels_mask']
        no_pad_keys = ['desc_vectors']
    ...
    def keep_for_metrics_fn(batch, output):
        ...
        # берём последний токен (desc/CLS)
        y_true = labels[:, -1, 0]      # (B*N,)
        y_pred = logits[:, -1, 0]      # (B*N,)
        mask   = masks[:, -1, 0] > 0   # (B*N,)

        # применяем маску
        y_true = y_true[mask]
        y_pred = y_pred[mask]

        preds = y_pred.unsqueeze(1)   # (M,1)
        target = y_true.unsqueeze(1)  # (M,1)
        reduce_dims = (0, 1)
        ...
        # gene_id / keys / dataset_description вынимаем из batch и фильтруем той же mask
        ...
        # Применим маску (torch.bool → список индексов)
        mask_idx = mask.nonzero(as_tuple=True)[0].tolist()
        data['tpm_true'] = y_true.tolist()
        data['tpm_preds'] = y_pred.tolist()
        data['gene_id'] = [flat_gene_id[i] for i in mask_idx]
        data['keys_id'] = [flat_keys_id[i] for i in mask_idx]
