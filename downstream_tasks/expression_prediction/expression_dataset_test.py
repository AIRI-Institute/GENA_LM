...
class ExpressionDataset(Dataset):
    def __init__(
        ...
        self._log_time("init: start")

        assert sys.version_info >= (3, 8), "Python version must be 3.8 or higher"  # rely on dict order
        ...
        self._log_time("tokenizer loaded")
        ...
        self._log_time("base params set")

        # read mapping paths
        self.read_paths()
        self._log_time("paths read")
        ...
        self._log_time("hash prefix resolved")

        forward_genes = pd.read_csv(forward_intervals_path, sep = '\t') if forward_intervals_path is not None else pd.DataFrame()
        assert "strand" not in forward_genes.columns.values, "forward_intervals_path must not contain strand column"
        ...
        assert "strand" not in reverse_genes.columns.values, "reverse_intervals_path must not contain strand column"
        reverse_genes["strand"] = "-"
        self.genes = pd.concat([forward_genes, reverse_genes])
        self._log_time("intervals loaded")
        ...
        self._log_time("keys/chunks computed")
        ...
        if cell_type_specific_samples_path is not None:
            ...
            if len(self.cell_type_specific_samples) == 0:
                ...
                self.cell_type_specific_samples = self.cell_type_specific_samples.groupby("gene_id")["cell_id"].apply(list).to_dict()
                self.n_cell_chunks = 1
        else:
            self.N_cell_type_specific_samples = 0
        self._log_time("cell-type-specific sampling processed")

        self.files_opened = False

        # genome fasta
        self.sequences = FastaFile(self.genome)
        self._log_time("fasta opened")
        ...
        self._log_time("tokens cache ready")

        # description (desc vectors)
        with open(text_data_path, "rb") as f:
            self.desc_data = pickle.load(f)
        self._log_time("description loaded")

        # bigWig cache/signals
        if self.bw:
            ...
            self._log_time("signals cache ready")
        ...
        if self.tpm:
            ...
            if os.path.exists(tpm_hash_path):
                ...
                self.tpm_cache = {}
                pbar = tqdm.tqdm(self.paths.items(), total=len(self.paths), desc="Reading TPM csvs")
                for key, (bw_paths, tpm_path) in pbar:
            ...
            self._log_time("TPM cache ready")
        ...
        self._log_time("valid indices computed")
        self._log_time("init: done")

    # --- timing helper methods ---
    def _log_time(self, label: str):
        now = datetime.datetime.now()
        elapsed_block = (now - self._checkpoint_time).total_seconds()
        elapsed_total = (now - self._start_time).total_seconds()
        self.logger.info(f"[{now}] {label} | +{elapsed_block:.2f}s since last | {elapsed_total:.2f}s since start")
        self._checkpoint_time = now

    def _log_time_local(self, label: str, start_dt: datetime.datetime, last_dt: datetime.datetime):
        now = datetime.datetime.now()
        self.logger.info(
            f"[{now}] {label} | +{(now - last_dt).total_seconds():.2f}s since last | "
            f"{(now - start_dt).total_seconds():.2f}s since start"
        )
        return now

    # Вычисляем список валидных индексов
    def _compute_valid_indices(self, report_every:int = 1000):
        ...
        for idx in range(len(self.genes)):
            ...
            if count % report_every == 0:
                last_dt = self._log_time_local(f"_compute_valid_indices progress: {count}/{len(self.genes)}", start_dt, last_dt)

        self.logger.info(f"Found {len(self.valid_indices)} valid samples out of {len(self.genes)}")
        self._log_time("_compute_valid_indices finished")
    ...
    def get_hash_path(self):
        ...
        if self.token_len_for_fetch != 8:  # legacy default
    ...
    def read_paths(self):
        ...
        last = t0
        self.paths = {}
        self.logger.info(f"[{t0}] Reading paths from {self.targets_path}")
        df = pd.read_csv(self.targets_path)
        last = self._log_time_local("read_paths: csv loaded", t0, last)

        self.dataset_description = df.iloc[0]['dataset_description']
        last = self._log_time_local("read_paths: dataset_description extracted", t0, last)

        assert not df["id"].duplicated().any(), "Found duplicated id in targets_path"
        last = self._log_time_local("read_paths: ids uniqueness checked", t0, last)
        ...
        last = self._log_time_local("read_paths: bw paths built", t0, last)
        ...
        last = self._log_time_local("read_paths: tpm paths built", t0, last)

        self.files_opened = False
        self._log_time_local("read_paths: finished", t0, last)

    def precompute_tokenization(self, progress_every:int = 250):
        ...
        try:
            with h5py.File(temp_path, "w") as h5f:
                ...
                for idx in range(len(self.genes)):
                    ...
                    if idx % progress_every == 0 and idx > 0:
                        last = self._log_time_local(f"precompute_tokenization progress: {idx}/{len(self.genes)}", t0, last)
            ...
            self.h5_cache = h5py.File(self.h5_cache_path, "r")
            self._log_time_local("precompute_tokenization: file written", t0, last)
        ...
        self._log_time("precompute_tokenization finished")
    ...
    def open_files(self):
        ...
        self._log_time_local("open_files finished", t0, last)
    ...
    def precompute_signals(self, progress_every:int = 100):
        ...
        try:
            if not self.files_opened:
                self.open_files()
                last = self._log_time_local("precompute_signals: files opened", t0, last)
            ...
            with h5py.File(temp_path, "w") as h5f:
                ...
                for idx in range(len(self.genes)):
                    ...
                    for i, (key, bw_h) in enumerate(self.bigWigHandlers.items()):
                        track_signals = self.process_region_signals(bw_h[strand], chrom, starts[:l], ends[:l], l, strand)
                    ...
                    if idx % progress_every == 0 and idx > 0:
                        last = self._log_time_local(f"precompute_signals progress: {idx}/{len(self.genes)}", t0, last)
            ...
            self.signals_cache = h5py.File(self.signals_cache_path, "r")
            self._log_time_local("precompute_signals: file written", t0, last)
        ...
        self._log_time("precompute_signals finished")
    ...
    def __getitem__(self, idx):
        ...
        if not self.files_opened and self.bw:
            self.open_files()
            last = self._log_time_local("__getitem__: open_files (on-demand)", t0, last)

        gene_id = self.genes.iloc[original_idx]['gene_id']
        gene_group = self.h5_cache[gene_id]
        last = self._log_time_local("__getitem__: fetched H5 group", t0, last)
        ...
        if self.N_cell_type_specific_samples > 0:
            if gene_id in self.cell_type_specific_samples:
                ...
                if len(_cell_type_specific_samples) > self.N_cell_type_specific_samples:
                    _cell_type_specific_samples = np.random.choice(_cell_type_specific_samples,
                                                                   ...
                                                                   replace=False)
                if len(_cell_type_specific_samples) < self.n_keys:
                ...
                elif len(_cell_type_specific_samples) > self.n_keys:
            ...
            selected_keys = self.selected_keys_chunks[chunk_idx]
        last = self._log_time_local("__getitem__: selected_keys ready", t0, last)
        ...
        last = self._log_time_local("__getitem__: base features built", t0, last)

        if self.bw:
            # Load from cache
            if self.signals_cache is not None:
                gene_id_cache = self.genes.iloc[original_idx]['gene_id']
                signals_group = self.signals_cache[gene_id_cache]
                bigwig_signals = np.array(signals_group['signals'])
            else:
                bigwig_signals = np.zeros((l, len(self.bigWigHandlers)), dtype=np.float32)
                for i2, (key, bw_h) in enumerate(self.bigWigHandlers.items()):
                    track_signals = self.process_region_signals(bw_h[strand], chrom, starts[:l], ends[:l], l, strand)
                    bigwig_signals[:, i2] = track_signals

            chunk_signals = np.zeros((l, self.n_keys), dtype=np.float32)
            chunk_mask = np.zeros((l, self.n_keys), dtype=bool)

            for i2, key in enumerate(selected_keys):
                if key in self.bigWigHandlers:
                    chunk_signals[:, i2] = bigwig_signals[:, list(self.bigWigHandlers.keys()).index(key)]
                    chunk_mask[:, i2] = True
                else:
        ...
        last = self._log_time_local("__getitem__: labels prepared", t0, last)
        ...
        if reverse == 0 :
            ...
            features["end"] = ends[0]
        last = self._log_time_local("__getitem__: coords computed", t0, last)
        ...
        if self.tpm:
            for i2, key in enumerate(selected_keys):
                if gene_id in self.tpm_lookup[key].index:
                    tpm_values[i2] = self.tpm_lookup[key].loc[gene_id].iloc[0]
        ...
        last = self._log_time_local("__getitem__: tpm prepared", t0, last)

        # desc vectors (only selected keys, padded)
        desc_vectors_list = []
        ...
        for i2, vec in enumerate(desc_vectors_list):
            desc_vectors[i2] = vec

        valid_tpm_count = np.count_nonzero(~np.isnan(tpm_values))
        features["dataset_description"] = np.array([self.dataset_description] * valid_tpm_count)
        features["desc_vectors"] = torch.tensor(desc_vectors, dtype=torch.float)
        if len(selected_keys) >= valid_tpm_count:
            features["selected_keys"] = selected_keys[:valid_tpm_count]
        else:
            raise ValueError("selected_keys меньше valid_tpm_count!")
        self._log_time_local("__getitem__: desc vectors ready (done)", t0, last)
