...
def main():
   ...
   # alogger = get_logger('')  # accelerate logger (привязан к процессу)
    ...
    def collate_fn(batch):
        pad_keys = ['input_ids', 'attention_mask', 'labels', 'labels_mask']
    ...
    def keep_for_metrics_fn(batch, output):
        ...
        y_true = labels[:, 0, 0]      # (B*N,)
        y_pred = logits[:, 0, 0]      # (B*N,)
        mask   = masks[:, 0, 0] > 0   # (B*N,)
    ...
    def make_metrics_fn(model_path, save_predictions=False):
        def metrics_fn(data):
            ...
            for dataset_desc in df['dataset_description'].unique():
                ...
                # df_true, df_pred = df_true.align(df_pred, join="inner", axis=None)
