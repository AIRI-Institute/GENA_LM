...
class ExpressionCounts(BertPreTrainedModel):
    ...
    def __init__(
        ...
        if len(missing_k) != 0:
            print('%s were not loaded from checkpoint! These parameters were randomly initialized.' % missing_k)
        if len(unexpected_k) != 0:
            print('%s were found in checkpoint, but model is not expecting them!' % unexpected_k)

        # 1.1) SciBERT для дескрипторов
    ...
    def forward(
        ...
        Размерности:
          ...
          - attention_mask: (B, seq_len)
          - token_type_ids: (B, seq_len) [опционально]
          - desc_input_ids: (B, N, desc_seq_len)
          - desc_attention_mask: (B, N, desc_seq_len)
          - labels: (B, seq_len, N) -> приводим к (B*N, seq_len, 1)
          - labels_mask: (B, seq_len, N) -> приводим к (B*N, seq_len, 1)
        
        Шаги:
          1) Прогоняем через GENA -> (B, seq_len, hidden_size)
          2) Прогоняем desc_input_ids через SciBERT -> (B, N, desc_seq_len, hidden_size_desc), берём CLS ([:, :, 0, :]) -> (B, N, hidden_size_desc)
          3) Расширяем выход -> (B, N, seq_len, hidden_size)
          4) Прогоняем desc_vectors через MLP 
          5) Складываем desc_vectors с CLS
          6) Превращаем (B, N, seq_len, hidden_size) -> (B*N, seq_len, hidden_size)
          7) Прогоняем через Encoder
          8) classifier -> (B*N, seq_len, 1)
          9) Меняем labels и labels_mask -> (B*N, seq_len, 1), считаем loss
        """
        ...
        sci_bert_outputs = self.sci_bert(
            input_ids=desc_input_ids_2d,  # (B*N, desc_seq_len)
            attention_mask=desc_attention_mask_2d,  # (B*N, desc_seq_len)
            return_dict=True
        )
        # Берём CLS токен
        desc_vectors_2d = sci_bert_outputs.last_hidden_state[:, 0, :].reshape(B, N, self.hidden_size_desc)
