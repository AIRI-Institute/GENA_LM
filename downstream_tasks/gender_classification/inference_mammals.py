...
from mammals_gender_dataset import (
    MultiSpeciesGenderDataChunkedDataset,
    collate_fn,
    worker_init_fn,
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Gender classification inference script"
    )
    ...
    # Model configuration
    parser.add_argument(
        "--n_chunks", type=int, default=16, help="Number of chunks per sample"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--force_sampling_from_y",
        type=bool,
        default=False,
        help="Forcing sampling from y chromosome",
    )
    ...
    parser.add_argument(
        "--chrX_ratio", type=float, default=None, help="Ratio for chromosome X sampling"
    )
    parser.add_argument(
        "--chrY_ratio", type=float, default=None, help="Ratio for chromosome Y sampling"
    )

    parser.add_argument(
        "--force_species", type=str, default=None, help="Species for evaluation"
    )

    parser.add_argument(
        "--output_file_prefix", type=str, default="", help="Output filename prefix"
    )

    parser.add_argument("--save_probs", action="store_true", help="Save probabilities")

    parser.add_argument(
        "--metrics_output_file_prefix",
        type=str,
    ...
    for batch in tqdm(dataloader, total=N):
        ...
        with autocast(dtype=torch.bfloat16):
            with torch.no_grad():
                ...
                losses.extend(
                    outputs["per_sample_loss"].float().cpu().numpy().reshape(-1)
                )
...
def run_inference(args):
    ...
    for species in species_list:
        ...
        losses = results["sample_ids_losses"][species]
        logger.info(
            f"{species}: {len(probs)} samples \n"
            f"threshold: {(probs>0.5).sum() / len(probs):.2f}\n"
            f"mean BCE loss: {losses.mean():.4f}"
        )
            ...
            pickle.dump(
                {
                    ...
                    "sample_ids_sampled_chromosomes": results[
                        "sample_ids_sampled_chromosomes"
                    ],
                },
                f,
    ...
    return (
        results["sample_ids_probs"],
        results["sample_ids_labels"],
        results["sample_ids_sampled_chromosomes"],
        results["sample_ids_losses"],
    )
...
def run_evaluation(
    ...
    new_sample_ids_sampled_chromosomes = {
        species + "_male": [],
        species + "_female": [],
    }
    ...
    for i, elem in enumerate(sample_ids_probs[species]):
        if sample_ids_labels[species][i] == 0:
            ...
            new_sample_ids_sampled_chromosomes[species + "_male"].append(
                sample_ids_sampled_chromosomes[species][i]
            )

        elif sample_ids_labels[species][i] == 1:
            new_sample_ids_probs[species + "_female"].append(elem)
            new_sample_ids_sampled_chromosomes[species + "_female"].append(
                sample_ids_sampled_chromosomes[species][i]
            )
    ...
    for key in new_sample_ids_sampled_chromosomes.keys():
        new_sample_ids_sampled_chromosomes[key] = np.array(
            new_sample_ids_sampled_chromosomes[key]
        )
    ...
    accs, stds = calculate_metrics(
        new_sample_ids_labels,
        new_sample_ids_probs,
        new_sample_ids_sampled_chromosomes,
        N2threshold=N2thr,
    )
    species_losses = np.asarray(sample_ids_losses[species], dtype=float)
            ...
            json.dumps(
                {
                    ...
                    "accs": accs,
                    "stds": stds,
                    "losses": species_losses.tolist(),
                    "mean_loss": float(species_losses.mean()),
                    "n_chunks": args.n_chunks,
                    "chunk_size": args.chunk_size,
...
if __name__ == "__main__":
    ...
    logger.info(
        f"Mean BCE loss: {float(np.asarray(list(sample_ids_losses.values())[0]).mean()):.4f}"
    )
