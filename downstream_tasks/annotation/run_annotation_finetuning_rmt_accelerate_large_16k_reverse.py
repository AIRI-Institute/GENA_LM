...
from downstream_tasks.annotation.transcript_GenePredictionDataset_reverse import AnnotationDataset
...
parser.add_argument('--backbone_checkpoint', type=str,
                    help='MLM pretrained checkpoint (default: None).')
...
if __name__ == '__main__':
    ...
    if args.num_mem_tokens is not None:
        ...
        if args.rmt_checkpoint is not None:
            ...
            missing_k, unexpected_k = model.load_state_dict(checkpoint, strict=False)
            if len(missing_k) != 0:
                logger.info(f'{missing_k} were not loaded from checkpoint! These parameters were randomly initialized. RMT.')
            if len(unexpected_k) != 0:
                logger.info(f'{unexpected_k} were found in checkpoint, but model is not expecting them! RMT.')
    ...
    pos_weight = torch.tensor([128.0, 11.8, 1.0, 33.6, 27.4, 11.7]) # torch.tensor([2.0, 0.05, 0.5])
    ...
    def batch_transform_fn(batch):
        ...
        return {
            ...
            'pos_weight': pos_weight.repeat(bs, seq_len, 1), #pos_weight.repeat(bs, seq_len, 1),
        }
