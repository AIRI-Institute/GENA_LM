...
class AnnotationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if initial_len_input_ids < self.max_seq_len:
            labels = np.array([[-100, -100, -100, -100, -100, -100, -100] if input_ids[idx] <=5 else i for idx, i in enumerate(labels)] + [[-100, -100, -100, -100, -100, -100, -100]] * (self.max_seq_len - len(labels)))
        else:
            labels = np.array([[-100, -100, -100, -100, -100, -100, -100] if input_ids[idx] <=5 else i for idx, i in enumerate(labels[:self.max_seq_len-1, :])] + [[-100, -100, -100, -100, -100, -100, -100]])
