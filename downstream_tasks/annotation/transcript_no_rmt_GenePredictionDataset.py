...
class AnnotationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_ids = np.array(
            self.data[sample_name]["input_ids"][1:-1]
        )
        ...
        max_random_pos = len(input_ids) - self.max_seq_len
        random_pos = np.random.randint(0, max_random_pos) if max_random_pos > 0 else 0
        
        input_ids = input_ids[random_pos:random_pos+self.max_seq_len]
        labels = labels[random_pos:random_pos+self.max_seq_len, :]
        ...
        # assert np.sum(labels_mask) == self.max_seq_len
