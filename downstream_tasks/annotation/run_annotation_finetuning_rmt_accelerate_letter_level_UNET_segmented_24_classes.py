...
from downstream_tasks.annotation.transcript_GenePredictionDataset_letter_level_UNET_segmented_24_classes import AnnotationDataset
...
parser.add_argument('--unet_sub_model_input_size', type=int, default=None, help='maximal input size of the sub model model')
...
if __name__ == '__main__':
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=per_worker_batch_size, sampler=train_sampler, persistent_workers=True, **kwargs) # persistent_workers=True for seamless epoch switch
    ...
    if args.valid_data_path:
        ...
        valid_dataset = AnnotationDataset(valid_data_path, max_seq_len=args.input_seq_len, tokenizer=tokenizer, max_atcg_seq_len=args.letter_level_input_seq_len)
    ...
    #     logger.info(f'loading pre-trained backbone from {args.init_checkpoint}')
    #     checkpoint = torch.load(args.init_checkpoint, map_location='cpu')
    #     if args.init_checkpoint[-4:] == '.bin':
    #         missing_k, unexpected_k = model.load_state_dict(checkpoint, strict=False)
    #     elif args.init_checkpoint[-4:] == '.pth':
    ...
    if args.num_mem_tokens is not None:
        rmt_config = {
            ...
            'unet_sub_model_input_size': args.unet_sub_model_input_size,
        ...
        if args.backbone_checkpoint is not None:
            ...
            if args.backbone_checkpoint[-4:] == '.bin':
                ...
                checkpoint = torch.load(args.backbone_checkpoint, map_location='cpu')['model_state_dict']
            # checkpoint = torch.load(args.backbone_checkpoint, map_location='cpu')
            checkpoint_new = dict()
        ...
        if args.full_checkpoint is not None:
            ...
            missing_k, unexpected_k = model.load_state_dict(checkpoint, strict=False)
            if len(missing_k) != 0:
                logger.info(f'{missing_k} were not loaded from checkpoint! These parameters were randomly initialized. Full.')
            if len(unexpected_k) != 0:
                logger.info(f'{unexpected_k} were found in checkpoint, but model is not expecting them! Full.')
    ...
    pos_weight = torch.tensor([1.0]*9) # torch.tensor([128.0, 11.8, 1.0, 33.6, 27.4, 11.7]) # torch.tensor([128.0, 11.8, 1.0, 33.6, 27.4, 11.7]) # torch.tensor([2.0, 0.05, 0.5])
    ...
    def metrics_fn(data):
        ...
        # fixed class order for the C channel
        # labels = ['EF', 'DF', 'IF', 'AF', 'ER', 'DR', 'IR', 'AR', 'NC']

        # ---------- 1. collect valid tokens ------------------------------------
        preds, trues = [], []
        for b in range(data['predictions'].shape[0]):
            mask = data['letter_level_labels_mask'][b]        # (T,)
            preds.append(data['predictions'][b][mask])        # (N_valid, C)
            trues.append(data['letter_level_labels'][b][mask])

        preds = torch.sigmoid(torch.cat(preds, dim=0))        # (N, C) probabilities
        trues = torch.cat(trues, dim=0)                       # (N, C) one-hot

        # ---------- 2. hard class predictions -------------------------------
        y_pred = preds.argmax(dim=1).cpu().numpy()
        y_true = trues.argmax(dim=1).cpu().numpy()

        # ---------- 3. MCC calculations --------------------------------------
        metrics = {'mcc_overall': matthews_corrcoef(y_true, y_pred)}
        ...
        #     metrics[f'mcc_{lbl}'] = mcc
        #     per_class_mcc.append(mcc)

        # # mean of per-class MCC values
        # metrics['mcc_mean'] = float(sum(per_class_mcc) / len(per_class_mcc))
    ...
    #     label_dict = {0: '5UTR', 1: 'EXON', 2: 'INTRON', 3: '3UTR', 4: 'CDS'} # {0: 'CDS-0', 1:'CDS-1', 2:'CDS-2', 3:'CDS-skip', 4:'intron-0', 5:'intron-1', 6:'intron-2', 7:'ASS-0', 8:'ASS-1', 9:'ASS-2', 10:'DSS-0', 11:'DSS-1', 12:'DSS-2', 13:'START', 14:'STOP', 15:'nc_exon_plus', 16:'nc_exon_minus', 17:'nc_intron_plus', 18:'nc_intron_minus', 19:'nc_ASS', 20:'nc_DSS', 21:'TSS' , 22:'PolyA', 23:'IR'}
