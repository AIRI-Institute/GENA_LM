...
#             overlap_mask = np.ones(token_atcg_raw.shape[0])
...
#         class_labels = labels.squeeze().astype(int)
#         mask_1 = class_labels == 23
#         mask_0 = (class_labels != 23) & (class_labels != -100)
#         class_labels[mask_1] = 1
#         class_labels[mask_0] = 0
#         # print(np.unique(class_labels))
#         one_hot_labels = np.full((class_labels.shape[0], 2), -100, dtype=np.float32)
#         valid_mask = class_labels != -100
#         one_hot_labels[valid_mask] = np.eye(2)[class_labels[valid_mask]]
...
class AnnotationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # assert False
        token_atcg_raw  = sample["token_atcg"]             # 1‑D int array, shape (L,)
        overlap_mask    = sample.get("n_mask", None)     # only in new “sample_” data
        # if overlap_mask is None:
        #     overlap_mask = np.ones(token_atcg_raw.shape[0])
        ...
        if labels.shape[0] < self.max_atcg_seq_len:
            labels = np.pad(
                labels,
                pad_width=((self.max_atcg_seq_len - labels.shape[0] - 1, 1), (0, 0)),
                mode='constant',
                constant_values=-100
            )
        ...
        return {
            "input_ids":               token_atcg_bpe.squeeze(),          # BPE IDs
            "letter_level_labels":     labels.astype(float),    # int → float as before
            "letter_level_labels_mask": letter_level_mask.squeeze()
        }
