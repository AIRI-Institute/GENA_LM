...
class AnnotationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        N = labels.shape[0]
        result_labels = np.full(N, 0, dtype=int) # 1
        # result_labels[labels[:, 4] == 1] = 4
        # result_labels[labels[:, 3] == 1] = 3
        result_labels[labels[:, 2] == 1] = 1 # 2
        # result_labels[labels[:, 0] == 1] = 0
        ...
        if labels.shape[0] < self.max_atcg_seq_len:
            labels = np.pad(labels, pad_width=((self.max_atcg_seq_len - labels.shape[0] - 1, 1)), mode='constant', constant_values=-100)
