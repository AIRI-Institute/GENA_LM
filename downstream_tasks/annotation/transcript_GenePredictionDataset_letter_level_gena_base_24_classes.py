...
class AnnotationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(token_repeater) < self.max_atcg_seq_len:
            labels = np.concatenate((labels, np.full((1, self.max_atcg_seq_len - len(token_repeater), 24), -100)), axis=1)
        ...
        return {
            "input_ids": input_ids.squeeze().astype(int),
            "token_type_ids": token_type_ids.squeeze().astype(int),
            "attention_mask": attention_mask.squeeze().astype(int),
