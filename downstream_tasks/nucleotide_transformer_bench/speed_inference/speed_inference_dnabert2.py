...
def try_batch_size_subprocess(ctx, model_id, seq_len, batch_size, gpu, vocab_size, min_token):
    q = ctx.Queue()
    p = ctx.Process(
        target=_try_batch_worker,
        args=(q, model_id, seq_len, batch_size, gpu, vocab_size, min_token),
    )
    p.start()
    p.join()
...
def auto_find_max_batch(ctx, model_id, seq_len, gpu, vocab_size, max_batch, min_token, log=True):
    lo = 0
    hi = 1
    ...
    while hi <= max_batch:
        ok = try_batch_size_subprocess(ctx, model_id, seq_len, hi, gpu, vocab_size, min_token)
        if log:
    ...
    while lo + 1 < hi:
        mid = (lo + hi) // 2
        ok = try_batch_size_subprocess(ctx, model_id, seq_len, mid, gpu, vocab_size, min_token)
        if log:
