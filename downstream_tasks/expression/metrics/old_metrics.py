...
def keep_for_metrics_fn(batch, output):
    ...
    rmt_labels_masks_segm = [
        [el.detach().cpu().to(torch.bool) for el in s]
        for s in output["labels_mask_reshaped"]
    ]
    ...
    for loss_component in loss_components:
        if (
            f"loss_{loss_component}" in output
            and output[f"loss_{loss_component}"] is not None
        ):
            data[f"loss_{loss_component}"] = (
                output[f"loss_{loss_component}"].detach().cpu()
            )
    ...
    data["_true_squared"] = torch.sum(torch.square(target), dim=reduce_dims).unsqueeze(
        0
    )
...
class OldMetrics:
    ...
    def compute(self):
        ...
        dataset_description_data = accelerate.utils.gather_object(
    ...
    def metrics_fn(self, data):
        ...
        covariance = (
            data["_product"]
            - true_mean * data["_pred"]
            - pred_mean * data["_true"]
        ...
        for loss_component in loss_components:
            if (
                f"loss_{loss_component}" in data
                and data[f"loss_{loss_component}"] is not None
            ):
                metrics[f"loss_{loss_component}"] = torch.mean(
                    data[f"loss_{loss_component}"]
                ).item()
            ...
            if not df_true.empty and not df_pred.empty:
                ...
                for gene in df_true.index:
                    ...
                    if len(gene_pred) > 1 and np.std(gene_pred) == 0:
                        logger.error(
                            f"dataset {dataset_desc} gene {gene} has all predicted values the same"
                        )
                        raise ValueError(
                            f"All predicted values for {gene} are the same. Are you missing cell type descriptions?"
                        )
                ...
                for cell_type in df_true.columns:
                    ...
                    if np.std(cell_pred.values) == 0 and len(cell_pred) > 1:
                        raise ValueError(
                            f"All predicted values for {cell_type} are the same"
                        )
                ...
                if gene_correlations:
                    metrics[f"pearson_corr_cells_{dataset_desc}"] = float(
                        np.mean(gene_correlations)
                    )
                if cell_correlations:
                    metrics[f"pearson_corr_genes_{dataset_desc}"] = float(
                        np.mean(cell_correlations)
                    )

                # 4 таргетных файла из borzoi
                target_metrics = calculate_target_genes_metrics(
                    df_true, df_pred, self.target_genes, logger=logger
                )
                metrics[f"pearson_corr_4_cells_borzoi_cells_{dataset_desc}"] = (
                    target_metrics["corr_selected_cell_types"]
                )
                metrics[f"pearson_corr_4_cells_borzoi_genes_{dataset_desc}"] = (
                    target_metrics["corr_selected_genes"]
                )
