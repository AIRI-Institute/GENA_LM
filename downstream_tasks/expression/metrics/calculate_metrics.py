...
def calculate_target_genes_metrics(df_true, df_pred, target_cell_types, logger=None):
    ...
    missing_in_true = [cell_type for cell_type in target_cell_types 
                      if cell_type not in df_true.columns]
    missing_in_pred = [cell_type for cell_type in target_cell_types 
                      if cell_type not in df_pred.columns]
    
    if missing_in_true or missing_in_pred:
        if missing_in_true:
            logger.debug(f"Following cell types were not found in true data: {missing_in_true}")
        if missing_in_pred:
            logger.debug(f"Following cell types were not found in predicted data: {missing_in_pred}")
    ...
    for cell_type in df_true_target.columns:
        ...
        if len(cell_true) > 1:
            try:
                assert np.std(cell_pred) != 0 and np.std(cell_true) != 0, f"std(cell_pred) or std(cell_true) is 0 for cell type {cell_type}"
    ...
    metrics['corr_selected_cell_types'] = float(np.mean(gene_correlations)) if gene_correlations else np.nan
    metrics['corr_selected_genes'] = float(np.mean(cell_correlations)) if cell_correlations else np.nan
