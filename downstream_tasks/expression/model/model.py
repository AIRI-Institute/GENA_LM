...
class ExpressionCountsModel(BertPreTrainedModel):
    ...
    def __init__(
        ...
        if hf:
            ...
            missing_k, unexpected_k = self.bert.load_state_dict(
                updated_state_dict, strict=False
            )

        if len(missing_k) != 0:
            print(
                f"{missing_k} were not loaded from checkpoint! These parameters were randomly initialized."
            )
        if len(unexpected_k) != 0:
            print(
                f"{unexpected_k} were found in checkpoint, but model is not expecting them!"
            )
    ...
    def save_cell_type_embeddings(self, save_path, cell_type_names=None):
        ...
        embeddings = (
            self.cell_type_embedding.weight.data.cpu()
        ...
        if cell_type_names is not None:
            if len(cell_type_names) != embeddings.shape[0]:
                print(
                    f"Warning: Number of cell type names ({len(cell_type_names)}) "
        ...
        print(
            f"Embedding statistics - Mean: {embeddings.mean().item():.4f}, Std: {embeddings.std().item():.4f}"
        )
    ...
    def load_cell_type_embeddings(self, load_path):
        ...
        if loaded_dict["embeddings"].shape != self.cell_type_embedding.weight.shape:
            raise ValueError(
                f"Loaded embedding shape {loaded_dict['embeddings'].shape} "
                f"doesn't match model embedding shape {self.cell_type_embedding.weight.shape}"
            )

        # Load the embeddings into the model
        self.cell_type_embedding.weight.data = loaded_dict["embeddings"].to(
            self.cell_type_embedding.weight.device
        )
