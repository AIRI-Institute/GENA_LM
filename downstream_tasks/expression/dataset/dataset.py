...
def convert_fm_relative_path_to_absolute_path(path, file_mappings_path):
    ...
    if os.path.isfile(file_mappings_path):
        base_path = os.path.abspath(os.path.dirname(file_mappings_path))
    elif os.path.isdir(file_mappings_path):
        base_path = os.path.abspath(file_mappings_path)
    else:
        raise ValueError(f"Invalid base_path for metadata file: {file_mappings_path}")
...
class ExpressionDataset(Dataset):
    def __init__(
        ...
        )  # we use dicts and realay on order of keys

        if isinstance(gen_tokenizer, str):
            self.gen_tokenizer = AutoTokenizer.from_pretrained(
                gen_tokenizer, trust_remote_code=True
            )
        else:
        ...
        if fraction_of_cell_type_specific_tpm_samples == 0:
            ...
            assert cell_type_specific_samples_path is not None, (
                "cell_type_specific_samples_path must be provided if fraction_of_cell_type_specific_tpm_samples is not 0"
            )

        if cell_type_specific_samples_path is not None:
            self.N_cell_type_specific_samples = max(
                1, int(self.n_keys * fraction_of_cell_type_specific_tpm_samples)
            )
            self.cell_type_specific_samples = pd.read_csv(
                cell_type_specific_samples_path
            )
            self.cell_type_specific_samples.query(
                "cell_id in @self.all_keys", inplace=True
            )
            self.cell_type_specific_samples.query(
                "gene_id in @self.genes['gene_id'].values", inplace=True
            )
            self.logger.debug(
                f"Found {len(self.cell_type_specific_samples)} cell-type-specific samples"
            )
            if (
                len(self.cell_type_specific_samples) == 0
            ):  # this may happen, for example, if cell type specific samples are for another species
                self.logger.warning(
                    f"No cell-type-specific samples found for {self.targets_path} in {cell_type_specific_samples_path}"
                )
                self.cell_type_specific_samples = None
                self.cell_type_specific_samples_path = None
                self.N_cell_type_specific_samples = 0
            else:
                self.cell_type_specific_samples = (
                    self.cell_type_specific_samples.groupby("gene_id")["cell_id"]
                    .apply(list)
                    .to_dict()
                )  # gene_id -> list of cell_ids
        ...
        if self.tpm:
            assert all(self.paths[k][1] is not None for k in self.paths), (
            ...
            if os.path.exists(tpm_hash_path):
                ...
                assert len(self.tpm_lookup) == len(self.paths), (
                    "Number of tpm cache and paths are not the same"
                )
                assert all(key in self.tpm_lookup for key in self.paths.keys()), (
                    "All keys in paths must be in tpm cache"
                )
                for key, value in self.tpm_lookup.items():
                    assert pd.isna(value).sum().sum() == 0, (
                        f"TPM cache contains NaN values for {key}"
                    )
            else:
                self.tpm_cache = {}
    ...
    def _compute_valid_indices(self):
        ...
        self.logger.info(
            f"Found {len(self.valid_indices)} valid samples out of {len(self.genes)}"
        )
    ...
    def get_hash_path(self):
        ...
        if (
            self.token_len_for_fetch != 8
        ):  # 8 was default in first version of the dataset; TODO: remove at some point
    ...
    def get_tpm_hash_path(self):
        ...
        return (
            self.hash_prefix
            + ".tpm."
            + signals_hash_path[len(self.hash_prefix) + len(".signal.") :]
        )
    ...
    def read_paths(self):
        ...
        if self.bw:
            ...
            assert not pd.isna(df[forward_colname]).any(), (
                f"{forward_colname} is NaN for some targets"
            )
            assert not pd.isna(df[reverse_colname]).any(), (
                f"{reverse_colname} is NaN for some targets"
            )
            forward_paths = (
                df[forward_colname]
                .apply(
                    lambda x: convert_fm_relative_path_to_absolute_path(
                        x, self.targets_path
                    )
                )
                .values
            ...
            self.paths = {
                k: [{"+": forward_paths[ind], "-": reverse_paths[ind]}]
        ...
        if self.tpm:
            ...
            assert not pd.isna(df[tpm_colname]).any(), (
                f"{tpm_colname} is NaN for some targets"
            )
            tpm_paths = (
                df[tpm_colname]
                .apply(
            ...
            with h5py.File(temp_path, "w") as h5f:
                ...
                for idx in range(len(self.genes)):
                    ...
                    gene_group.create_dataset(
                        "input_ids", data=tokens_df["token_id"].values.astype(np.int32)
                    )
                    gene_group.create_dataset(
                        "starts", data=tokens_df["start"].values.astype(np.int64)
                    )
                    gene_group.create_dataset(
                        "ends", data=tokens_df["end"].values.astype(np.int64)
                    )
                    if len(tokens_df["chrom"]) > 0:
                        gene_group.create_dataset(
                            "chrom", data=tokens_df["chrom"].iloc[0].encode("utf-8")
                        )
                    else:
                        self.logger.warning(
                            f"No chromosomes found for gene {gene_id}:\n {self.genes.iloc[idx]}"
                        )
                        raise ValueError(
                            f"No chromosomes found for gene {gene_id}:\n {self.genes.iloc[idx]}. \n Possible reason - genome mismatch."
                        )
                    gene_group.attrs["strand"] = self.genes.iloc[idx]["strand"].encode(
                        "utf-8"
                    )
    ...
    def check_bw_genome_consistency(self, bw_handler, sequences):
        ...
        fasta_ref_lengths = {
            k: v for k, v in zip(sequences.references, sequences.lengths)
        }
        ...
        assert len(common_refs) > 0, (
            f"No common references found in genome and bigwig file. Genome: {fasta_ref_lengths.keys()}, Bigwig: {bw_ref_lengths.keys()}. Genome mismatch?"
        )
        ...
        for ref in common_refs:
            if fasta_ref_lengths[ref] != bw_ref_lengths[ref]:
                raise ValueError(
                    f"Length of {ref} in genome and bigwig file are different. Ref: {ref}, Genome: {fasta_ref_lengths[ref]}, Bigwig: {bw_ref_lengths[ref]}"
                )
    ...
    def open_files(self):
        if self.bw:
            ...
            for k, (v1, v2) in self.paths.items():
                try:
                    self.bigWigHandlers[k] = {
                        strand: bw.open(path) for strand, path in v1.items()
                    }
    ...
    def tokenize_genome(self, i):
        ...
        if self.num_before > 0:
            if reverse == 0:  # forward strand
                try:
                    sequence = self.sequences.fetch(
                        chrom,
                        max(start - self.num_before * self.token_len_for_fetch, 0),
                        start,
                    ).upper()
                ...
                try:
                    sequence = self.sequences.fetch(
                        ...
                        start,
                        min(
                            start + self.num_before * self.token_len_for_fetch,
                            chrom_length,
                        ),
                    ).upper()
            ...
            encoded_sequence = self.gen_tokenizer.encode_plus(
                sequence, return_offsets_mapping=True
            )
            encoded_sequence["input_ids"] = encoded_sequence["input_ids"][1:-1]
            encoded_sequence["offset_mapping"] = encoded_sequence["offset_mapping"][
                1:-1
            ]
            if len(encoded_sequence["input_ids"]) < self.num_before:
                self.logger.warning(
                    f"Trying to tokenize seq before TSS, but it's too short: {len(encoded_sequence['input_ids'])} < {self.num_before}; {chrom}: {start}-{end} ({strand})"
                )
        ...
        token_lengths_df = pd.DataFrame(
            token_lengths, columns=["token_id", "token", "length"]
        )
        token_lengths_df["start"] = (
            token_lengths_df["length"].cumsum().shift(fill_value=0) + start_gene
        )
    ...
    def process_region_signals(self, bw, chrom, starts, ends, l, strand):
        ...
        try:
            ...
            self.logger.error(
                f"Error processing signals for {chrom}:{region_start}-{region_end}: {e}"
            )
        ...
        try:
            ...
            with h5py.File(temp_path, "w") as h5f:
                ...
                for idx in range(len(self.genes)):
                    ...
                    bigwig_signals = np.zeros(
                        (l, len(self.bigWigHandlers)), dtype=np.float32
                    )

                    for i, (key, bw) in enumerate(self.bigWigHandlers.items()):
                        track_signals = self.process_region_signals(
                            bw[strand], chrom, starts[:l], ends[:l], l, strand
                        )
                        ...
                        if self.norm_bw:
                            try:
                                ...
                                if norm_factor > 0:
                                    ...
                                    self.logger.warning(
                                        f"Zero normalization factor for {key}, strand {strand}"
                                    )
                            except KeyError:
                                self.logger.warning(
                                    f"Missing normalization factor for {key}, strand {strand}"
                                )
    ...
    def __getitem__(self, idx):
        ...
        if self.N_cell_type_specific_samples > 0:
            if gene_id in self.cell_type_specific_samples:
                ...
                if len(_cell_type_specific_samples) > self.N_cell_type_specific_samples:
                    ...
                    _cell_type_specific_samples = np.random.choice(
                        _cell_type_specific_samples,
                ...
                if (
                    len(_cell_type_specific_samples) < self.n_keys
                ):  # add random non-cell-type-specific samples
                    _not_cell_type_specific_samples = [
                        key
                        for key in self.all_keys
                        if key not in _cell_type_specific_samples
                    ]
                    assert len(_not_cell_type_specific_samples) + len(
                        _cell_type_specific_samples
                    ) == len(self.all_keys)
                    ...
                    selected_keys = np.concatenate(
                        [_cell_type_specific_samples, selected_keys]
                    )
                elif (
                    len(_cell_type_specific_samples) > self.n_keys
                ):  # choose n_keys cell-type-specific samples
                    raise ValueError(
                        f"N of cell-type-specific samples for {gene_id} is greater than n_keys: {len(_cell_type_specific_samples)} > {self.n_keys}"
                    )
                else:
        ...
        if self.bw:
            ...
            for i, key in enumerate(selected_keys):
                if key in self.bigWigHandlers:
                    chunk_signals[:, i] = bigwig_signals[
                        :, list(self.bigWigHandlers.keys()).index(key)
                    ]
                    chunk_mask[:, i] = True
                else:
        ...
        features["dataset_deviation"] = torch.full_like(
            torch.from_numpy(tpm_values), np.nan, dtype=torch.float32
        )
        if self.tpm:
            for i, key in enumerate(selected_keys):
                if gene_id in self.tpm_lookup[key].index:
            ...
            if not np.all(np.isnan(tpm_values)):
                ...
                features["dataset_mean"] = torch.tensor(
                    dataset_mean, dtype=torch.float32
                )
                features["dataset_deviation"] = torch.from_numpy(
                    (tpm_values - dataset_mean) / dataset_mean
                ).float()
        ...
        if type(desc_vectors_list[0]) == int:
            ...
            desc_vectors = np.zeros(
                (self.n_keys, len(desc_vectors_list[0])), dtype=np.float32
            )
